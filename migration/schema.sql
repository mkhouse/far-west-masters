...
create table races (
  ...
  -- particular, which is published by usalpinemasters.org. Excluded from best-N and
  -- season standings, but still available for participation and messaging.
  ...
  -- live-timing.com race id, entered once when the race is scheduled. One id per
  -- race: regular Far West races combine both genders in a single live-timing
  -- race; only nationals are gender-split. Unrelated to the USSA code below, so
  -- it cannot be derived from the fall schedule. See live-timing-format.md.
  live_timing_id int,

  -- US Ski & Snowboard race code, e.g. 'M0074'. Assigned in the fall, gender-split.
...
where p.phone is not null
  and p.opted_out_at is null      -- inbound STOP; overrides everything else
  and p.sms_never    = false      -- suppressed by us
  and p.opt_in_at    is not null  -- member volunteered consent via the form
  and p.intro_sent_at is not null -- FWM confirmed it with the intro text
;
-- BOTH consent signals are required. An intro text alone records that FWM made
-- contact, not that the member agreed; a form opt-in alone has not been confirmed.
-- The intro text send itself must not use this view — it targets exactly the people
-- who have not yet passed the gate.
...
-- Role privileges
--
-- The project disables "automatically expose new tables", so nothing is handed to
-- the API roles by default. That is deliberate: this database holds member phone
-- numbers, and the anon key ships in the browser.
--
-- The consequence to know about: that setting withholds privileges from
-- **service_role** as well, not only anon and authenticated. Without the grants
-- below, server-side code gets "permission denied for table seasons" (42501).
--
-- So: service_role gets everything (it is the trusted server-side identity, and
-- already bypasses RLS by design); the browser-facing roles get nothing.
-- -----------------------------------------------------------------------------
